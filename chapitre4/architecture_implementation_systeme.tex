...
Les deux déclinaisons ne possèdent pas le même niveau de maturité. Java/Spring Boot constitue la référence la plus avancée, avec un processus complet, une réparation gouvernée, un assistant et un reporting. Angular est en développement actif : les agents principaux sont implémentés et la trajectoire progressive 11--21 constitue l'objectif d'extension. La preuve d'exécution versionnée retenue valide actuellement les stages 18 vers 19 et 19 vers 20 ; le stage 20 vers 21 est préparé mais non démarré. Le texte distingue donc les fonctionnalités validées, partielles et encore à consolider.
...
Avant de distinguer les chaînes propres à Java/Spring Boot et à Angular, il est nécessaire d'identifier le socle technique partagé par les deux déclinaisons. Ce socle regroupe les composants qui assurent le pilotage du workflow, la persistance de l'état, l'exposition des services et l'interaction avec le développeur. Le Tableau~\ref{tab:technologies-communes-ch4} présente ces technologies communes et précise leur rôle dans l'architecture globale.
...
Cette quatrième étape est encore en cours à la date de référence du rapport ; elle ne doit donc pas être présentée comme une démonstration déjà réalisée. La Figure~\ref{fig:evolution-technique-java-ch4} synthétise les quatre incréments successifs.
...
Le Tableau~\ref{tab:comparaison-demos-java-ch4} compare les capacités majeures de ces incréments et distingue clairement le statut réalisé des éléments planifiés pour J4.
...
L'agent d'analyse constitue le point d'entrée du raisonnement agentique. Il transforme les informations brutes du projet en une représentation structurée des versions, dépendances, configurations et risques ; cette sortie sert ensuite de base à l'agent de révision puis au planificateur. Sa qualité conditionne donc directement la pertinence des décisions prises en aval. Le Tableau~\ref{tab:agent-analyse-ch4} résume son contrat fonctionnel.
...
L'agent de révision de l'analyse introduit un premier contrôle indépendant avant toute planification. Son rôle n'est pas de refaire l'analyse, mais d'en vérifier la couverture, la cohérence et l'appui sur les preuves disponibles. Il se place ainsi entre l'analyseur et le planificateur afin d'éviter qu'une compréhension incomplète du projet ne soit transformée en plan d'exécution. Le Tableau~\ref{tab:agent-review-analyse-ch4} synthétise ce rôle.
...
L'agent de planification transforme une analyse validée en trajectoire de migration exploitable. Il ordonne les stages, explicite leurs dépendances et associe à chacun des conditions de validation. Il constitue le lien entre la compréhension du projet produite en amont et les opérations de transformation qui seront exécutées après révision et approbation. Le Tableau~\ref{tab:agent-planification-ch4} présente sa fiche synthétique.
...
L'agent de révision du plan agit comme second contrôle agentique avant l'exécution. Il vérifie que la trajectoire proposée respecte les prérequis, les compatibilités et l'ordre des transformations attendues. Sa position entre le planificateur et la décision humaine permet de présenter au développeur un plan déjà soumis à une vérification indépendante. Le Tableau~\ref{tab:agent-review-plan-ch4} en précise les responsabilités.
...
L'agent de transformation est le premier agent susceptible de produire des modifications candidates sur l'application migrée. Il intervient uniquement à partir d'un plan validé et dans un espace de travail isolé, sans autorité sur la source d'origine. Il relie ainsi le raisonnement de planification à l'exécution technique, tandis que la compilation et les tests déterministes restent responsables de la validation du résultat. Le Tableau~\ref{tab:agent-transformation-ch4} résume ces limites.
...
Le proposeur de réparation n'intervient qu'après un échec effectivement observé lors de l'exécution ou de la validation. À partir des journaux, de la commande en échec et d'un contexte borné, il produit un correctif candidat sans pouvoir l'appliquer. Il ouvre ainsi la boucle de réparation tout en restant séparé de l'agent qui révisera la proposition et de l'humain qui décidera de son application. Le Tableau~\ref{tab:agent-proposeur-reparation-ch4} formalise ce périmètre.
...
L'agent de révision de la réparation constitue le contrôle indépendant de la proposition de correctif. Il examine sa portée, sa cohérence avec l'échec observé et les risques qu'elle introduit ; il peut rejeter la proposition ou produire une version consolidée. Il se situe donc entre le proposeur et la décision humaine, ce qui empêche un même agent de générer puis de valider seul son propre correctif. Le Tableau~\ref{tab:agent-review-reparation-ch4} décrit précisément ce rôle.
...
L'assistant de migration joue un rôle transversal d'explication plutôt qu'un rôle d'exécution. Il exploite les états, événements et preuves déjà produits afin d'aider le développeur à comprendre l'avancement, les blocages et les actions disponibles. Contrairement aux agents du pipeline, il ne fait pas progresser la migration et ne dispose d'aucune autorité sur les décisions ou les fichiers. Le Tableau~\ref{tab:agent-assistant-ch4} synthétise ses responsabilités et ses limites.
...
Le frontend transmet une intention à l'API. Le backend valide le schéma, charge l'état et vérifie l'action. L'orchestrateur choisit ensuite le nœud autorisé. Les résultats sont persistés avant toute projection vers le cockpit. La Figure~\ref{fig:sequencement-orchestrateur-ch4} synthétise le séquencement principal.
...
Pour concrétiser ce séquencement, l'orchestrateur s'appuie sur des contrats techniques explicites entre l'API, l'état persistant et les services d'exécution. La représentation suivante illustre le contrat logique d'une décision. Les noms sont simplifiés pour le rapport ; ils ne constituent pas une copie exhaustive d'une route.
...
% ============================================================
\section{Backend, persistance et interface de supervision}

Le backend constitue la source d'autorité de la plateforme : il valide les requêtes, persiste les états et rattache les actions aux preuves produites. L'interface ne duplique pas cette logique ; elle expose au développeur une projection de l'état autoritaire afin de rendre le workflow compréhensible et pilotable. Les deux sous-parties suivantes présentent successivement ces responsabilités backend puis leur restitution dans le cockpit.

\subsection{Backend, persistance et preuves}
% ============================================================
...
% ============================================================
\subsection{Interface de supervision et captures}
% ============================================================

Le cockpit constitue la projection opérationnelle de l'état persistant de la migration. Il présente la trajectoire calculée, les profils d'exécution, les phases en cours, les résultats d'analyse et de planification, les points de validation, les journaux, les preuves et les propositions de réparation. Le frontend n'est pas une source de vérité autonome : il restitue les états et artefacts fournis par le backend et n'autorise que les actions rendues disponibles par le workflow.

Afin de documenter l'interface sans transformer ce chapitre en galerie d'écrans, cinq vues représentatives ont été retenues : trois pour Angular et deux pour Java/Spring Boot. Elles illustrent respectivement la préparation d'une migration, la supervision d'une phase agentique, l'examen d'une réparation gouvernée Angular, la qualification de l'environnement Java et l'examen d'une réparation gouvernée Java/Spring Boot.

\paragraph{Déclinaison Angular.}

La figure~\ref{fig:interface-angular-configuration-ch4} montre l'écran de préparation d'une migration Angular. La source détectée, la cible demandée et la trajectoire adjacente-major sont explicitées avant le démarrage. La source reste en lecture seule et l'interface expose également les contrôles de disponibilité nécessaires à la revue de préparation.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\textwidth]{"logos/Angular screen 1.png"}
    \caption{Configuration d'une migration Angular et projection de la trajectoire cible 11--21}
    \label{fig:interface-angular-configuration-ch4}
\end{figure}

Cette projection décrit une \textbf{trajectoire cible calculée} ; elle ne constitue pas une preuve que l'ensemble du parcours Angular 11--21 a été exécuté. Les résultats techniques retenus dans ce rapport restent ceux explicitement documentés au Chapitre V.

La figure~\ref{fig:interface-angular-analyse-ch4} illustre ensuite la supervision en temps réel de la phase d'analyse. Le développeur peut suivre les nœuds déterministes déjà validés, le passage au proposeur d'analyse, l'intervention du réviseur indépendant, la progression globale et les journaux produits pendant l'exécution.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\textwidth]{"logos/angular analyse .png"}
    \caption{Supervision en temps réel de la phase d'analyse Angular}
    \label{fig:interface-angular-analyse-ch4}
\end{figure}

Cette capture documente le comportement de l'interface de supervision et la séparation des rôles entre les étapes déterministes, le proposeur d'analyse et le réviseur indépendant.

La figure~\ref{fig:interface-angular-reparation-ch4} présente ensuite une réparation gouvernée déclenchée par une incompatibilité d'outillage. Le diagnostic est figé avant la génération du correctif ; le Main Repair LLM produit une proposition bornée, puis un réviseur indépendant contrôle sa cohérence et les risques associés. Le diff exact est affiché dans le cockpit et reste soumis à l'approbation humaine G10 avant toute application.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\textwidth]{"logos/angular diff 2.png"}
    \caption{Exemple de réparation gouvernée Angular avec proposition de diff et révision indépendante}
    \label{fig:interface-angular-reparation-ch4}
\end{figure}

Cette vue met en évidence la séparation entre génération, révision et décision humaine : la proposition reste un artefact contrôlé tant que le point de validation n'est pas approuvé.

\paragraph{Déclinaison Java/Spring Boot.}

La figure~\ref{fig:interface-java-configuration-ch4} présente la trajectoire Java/Spring Boot avant création de la migration gouvernée. Les quatre stages, les versions de Java associées, Maven, la politique de continuation, le niveau de preuve et les contrôles de disponibilité sont regroupés dans une même vue afin que le développeur puisse vérifier le contexte d'exécution avant le lancement.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\textwidth]{"logos/java 2 env look.png"}
    \caption{Configuration de la trajectoire Java/Spring Boot et vérification de l'environnement d'exécution}
    \label{fig:interface-java-configuration-ch4}
\end{figure}

Enfin, la figure~\ref{fig:capture-reparation-java-ch4} montre une vue de réparation gouvernée. Le diagnostic, la décision du réviseur, l'empreinte de l'artefact et le diff proposé sont présentés ensemble. Cette organisation permet au développeur d'examiner la proposition exacte avant toute application et matérialise la séparation entre proposition, révision et exécution déterministe.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\textwidth]{"logos/java diffs .png"}
    \caption{Supervision d'une proposition de réparation gouvernée Java/Spring Boot}
    \label{fig:capture-reparation-java-ch4}
\end{figure}

Ces captures complètent les descriptions architecturales par une vue concrète de l'interaction avec la Factory. Elles illustrent la configuration, la progression et la gouvernance visibles par le développeur ; elles ne remplacent pas les preuves techniques d'exécution, de compilation ou de test analysées au Chapitre V.

% ============================================================
\section{Robustesse, sécurité et validation technique}

Après avoir présenté l'exécution fonctionnelle du système, il reste à examiner les conditions qui influencent sa robustesse et son aptitude à être industrialisé. Cette partie regroupe les mécanismes de sécurité déjà présents, les difficultés techniques rencontrées et les preuves de validation disponibles, afin de distinguer clairement les protections implémentées des limites encore ouvertes.

\subsection{Authentification, sécurité et limites d'exploitation}
% ============================================================
...
% ============================================================
\subsection{Difficultés techniques et corrections}
% ============================================================

Les difficultés rencontrées ne relèvent pas d'un seul type de défaut : elles concernent aussi bien l'applicabilité des correctifs, l'autorité des commandes, les profils de runtime, la persistance que l'intégration du frontend. Le Tableau~\ref{tab:difficultes-corrections-ch4} synthétise des obstacles représentatifs, leurs causes et les décisions correctives utilisées pour stabiliser les prototypes.
...
% ============================================================
\subsection{Preuves techniques disponibles}
% ============================================================
...
Pour Angular, une migration manuelle d'Angular 18 vers 20 a d'abord été réalisée afin de comprendre les transformations, puis la Factory a été orientée vers une trajectoire 11--21. Les agents d'analyse, de révision, de planification et de transformation sont développés. La preuve d'exécution versionnée retenue établit deux stages scellés, 18 vers 19 puis 19 vers 20 ; le stage 20 vers 21 est préparé mais non démarré. Le processus complet jusqu'à Angular~21 demeure donc en validation.
...
Les fiches uniformes précisent les entrées, traitements, sorties, exemples, autorités et limites. Les contrats simplifiés, les preuves d'intégration et les difficultés techniques renforcent le caractère concret de la réalisation. Le chapitre suivant formalise les démonstrations et preuves retenues, puis évalue les résultats disponibles, la qualité du système, la comparaison avec le processus manuel et les limites de mesure.
