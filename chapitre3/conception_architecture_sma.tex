...
La Figure~\ref{fig:modele-sma-ch3} synthétise les catégories de composants et les limites d'autorité du modèle hybride retenu.
...
La spécialisation des agents n'est utile que si leur périmètre d'action reste explicite. Le Tableau~\ref{tab:autonomie-agents-ch3} synthétise, pour chaque rôle, la capacité de raisonnement, l'accès aux outils, les droits d'écriture et les points où une validation indépendante ou humaine demeure obligatoire.
...
Les responsabilités décrites précédemment doivent maintenant être replacées dans une vue d'ensemble afin de comprendre leurs interactions. Cette architecture globale montre comment le cockpit, l'API, l'orchestrateur, les agents, les services déterministes et les mécanismes de persistance coopèrent au sein d'une même chaîne d'exécution. Elle met également en évidence le principe d'autorité retenu : les agents produisent des analyses ou des propositions, tandis que l'état, les transitions et l'exécution restent contrôlés par les composants déterministes. La Figure~\ref{fig:architecture-sma-ch3} présente cette vue d'ensemble.
% ============================================================
...
Les agents ne maintiennent pas une conversation libre comme source de vérité. Ils échangent indirectement par des artefacts orchestrés. Un contrat logique contient : identifiant de migration, tâche, producteur, consommateur, référence de contexte, schéma attendu, empreintes, statut et erreurs. La Figure~\ref{fig:sequence-sma-ch3} illustre une séquence simplifiée de ces échanges.
...
L'orchestrateur reçoit une intention validée par l'API, charge l'état persistant, sélectionne le nœud autorisé, prépare son contexte, enregistre la sortie, ouvre éventuellement un point de validation et détermine la suite. La Figure~\ref{fig:orchestrateur-ch3} synthétise cette logique.
...
Une migration possède un état global et des états locaux pour les stages, commandes, points de validation et réparations. Le frontend présente une projection ; il ne décide pas de l'état. La Figure~\ref{fig:machine-etats-sma-ch3} présente une machine à états simplifiée.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{"logos/Figure 3.5.png"}
...
Les points de validation permettent d'approuver, rejeter, demander une révision ou annuler. Une décision est liée à l'état courant et à l'empreinte de l'artefact examiné. La Figure~\ref{fig:reparation-sma-ch3} représente la boucle de réparation gouvernée.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{"logos/Figure 3.6.png"}
